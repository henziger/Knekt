...
b)}function clearSettingIndications(){var a=$("#lang-select"),b=$("#file-select");a.removeClass("changed-setting");b.removeClass("changed-setting");a.children("option").removeClass("last-setting");b.children("option").removeClass("last-setting")};function getNGramCount(a,b,c){if("string"!==typeof a)throw TypeError("text must be a string");if("number"!==typeof b)throw TypeError("n must be a number");if(1>b)throw RangeError("n must be greater than or equal to one");if("number"!==typeof c)throw TypeError("count must be a number");if(0>c)throw RangeError("count must be non-negative");a=a.toUpperCase();for(var d={},e="",f=0;f<=a.length-b;f++)e=a.slice(f,f+b),d[e]=d[e]?d[e]+1:1;a=[];for(e in d)d.hasOwnProperty(e)&&a.push({ngram:e,num:d[e]});a.sort(function(a,
b){return b.num-a.num});return a.slice(0,c)}function createEmptyContext(a){for(var b=[],c=0;c<a;c++)b.push({ngram:"-",num:"-"});return b};function getBigramDistances(a,b){for(var c=[],d=a.indexOf(b);-1!==d;){for(var e=0;e=a.indexOf(b,e+d+1),-1!==e;)c.push(e-d);d=a.indexOf(b,d+1)}return c}function getBigramFactors(a){for(var b=getNGramCount(a,2,50),c=[{nr:2,count:0},{nr:3,count:0},{nr:4,count:0},{nr:5,count:0},{nr:6,count:0},{nr:7,count:0},{nr:8,count:0}],d=0;d<b.length;d++)for(var e=getBigramDistances(a,b[d].ngram),f=2;f<c.length+2;f++)for(var g=0;g<e.length;g++)0===e[g]%f&&(c[f-2].count+=1);return c};function memoize(a){var b={};return function(){var c=String(Hashcode.value(JSON.stringify(arguments))>>>0);return b[c]=b[c]||a.apply(this,arguments)}}function getValuesFromObject(a){if(null===a||"object"!==typeof a)throw new TypeError("Not an object!");var b=[],c;for(c in a)Object.prototype.hasOwnProperty.call(a,c)&&b.push(a[c]);return b}
...
c)(),!1):!0})};function buildList(a,b){for(var c=[],d=0;d<b;d++)c.push(a);return c}function calculateColumns(a,b){if("string"!==typeof b)throw new TypeError("text not a string");if("number"!==typeof a)throw new TypeError("nCols not a number");if(0>=a)throw new RangeError("nCols smaller than 1");for(var c=buildList("",a),d=0;d<b.length;d++)c[d%a]+=b[d];return c}function decryptTransposition(a,b,c){for(var d=calculateColumns(a,c),e="",f,g=0;g<c.length+a;g++)f=b[g%a],f=d[f][Math.floor(g/a)],e+=f?f:"";return e}
function padText(a,b){var c=b;if(0!==b.length%a)for(var d=0;d<a-b.length%a;d++)c+=" ";return c}function getHandlebarsColumns(a,b){for(var c={column:[]},d=calculateColumns(a,b),e=0;e<a;e++)c.column.push({number:e+1,letters:d[e].split("")});return c};$(function(){initAppState();initInterface();initBigramFactorMngr();initNGramTablesMngr();initStatChartsMngr();initHomeMngr();initViewMngr();initSubstMngr();initTransMngr();initVigMngr();initAbout()});var nCols=4;function updateResultText(){for(var a=document.querySelectorAll("#column-container > .trans-column"),b=[],c,d=0;d<a.length;d++)c=parseInt(a[d].getAttribute("data-index"))-1,b.push(c);a=decryptTransposition(nCols,b,appState.getFileText());$("#trans-result").html(a)}function columnIncrDecr(a){appState.fileIsSelected()&&(nCols+=a,9<nCols?nCols=9:2>nCols?nCols=2:generateNewColumns())}
...
"No file loaded")},initStatChartsMngr=function(){document.body.addEventListener("languageChange",function(a){plotGraphs(appState.getLang())});document.body.addEventListener("fileChange",function(a){plotGraphs(appState.getLang())});plotGraphs(appState.getLang())};function initAbout(){var a=$("#about-button"),b=$("#about-close-button"),c=$("#about-overlay"),d=function(a){!c.is(":visible")||$(a.target).closest("#about-overlay").length||$(a.target).is("#about-button")||(c.hide(),$(document).unbind("click",d))};a.click(function(){c.show();$(document).click(d)});b.click(function(){c.hide();$(document).unbind("click",d)});$(document).keyup(function(a){27===a.keyCode&&(c.hide(),$(document).unbind("click",d))})};var appConfig=appConfig||{};appConfig.CRYPTO_TEXT_PATH="res";appConfig.TEXT_FILE_COUNT=8;function initSubstMngr(){substResetTool();document.body.addEventListener("fileChange",function(a){substResetTool("fileChange",a.detail)});document.body.addEventListener("languageChange",function(){substResetTool("resetDecrypted")})}
...
g=g.toInnerHTMLArray();h(d,c);h(k,g);return{val:f}}function updateMessageDivs(a){if("object"!==typeof a)return!1;var b=$("#message-divs"),c=$(".decr-msg-row",b),d=$(".encr-msg-row",b).toInnerHTMLArray(),e=0;c.each(function(b,c){var h=decryptLine(d[b],a);h!==c.innerHTML&&(e++,$(c).html(h))});return!0}
function substResetTool(a,b){var c=initAlphabet(appState.getLang());"fileChange"==a&&b&&handlebarMessageDivs(b,c);handlebarAlphabetDivs(c);handlebarCryptobetDivs(c);setupInputCallbacks();"resetDecrypted"==a&&updateMessageDivs(c)}function substDrop(a){var b=a.dataTransfer;b&&a.target&&b.getData?(a.target.innerHTML=b.getData("source"),$(a.target).blur(),$(a.target).trigger("input")):alert("dropping not supported by this browser");a.preventDefault()}
function substDrag(a){var b=a.dataTransfer;b&&a.target&&b.setData?b.setData("source",a.target.innerHTML):(alert("drag-and-drop is not supported on this browser"),a.preventDefault())};
